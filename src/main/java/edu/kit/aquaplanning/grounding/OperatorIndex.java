...
public class OperatorIndex {
  ...
  /**
   * Maps the name of an operator to a mapping of (argument names to their
   * position within the operator's parameters).
   */
    ...
    for (Operator op : filteredOps) {
        ...
        for (int pos = 0; pos < op.getArguments().size(); pos++) {
          ...
          for (Argument constant : p.getConstants()) {
            if (p.isArgumentOfType(constant, op.getArgumentTypes().get(pos))) {
              ...
              } else if (pre.getArguments().contains(op.getArguments().get(pos)) && !predicateName.equals("=")) {
      ...
      while (!assignmentStack.isEmpty()) {
        ...
        if (decisionLevel == op.getArguments().size()) {
          ...
          if (!instantiatedOperators.getOrDefault(op.getName(), new ArgumentNode(argumentIds)).contains(args)) {
