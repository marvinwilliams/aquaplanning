  ...
  private boolean containsPartiallyInstantiatedArgs(List<Argument> args, int argPos) {
    ...
    if (argPos == args.size()) {
      ...
      if (args.get(argPos) == null || !args.get(argPos).isConstant()) {
        ...
        /*
         * TODO if (isLeafNode) return true; for (int id : children.keySet()) { if
         * (children.get(id).containsPartiallyInstantiatedArgs(args, argPos+1)) { return
         * true; } } // No valid instantiation // Are any arguments left to check? for
         * (int nextPos = argPos+1; nextPos < args.size(); nextPos++) { if
         * (args.get(nextPos) != null && args.get(nextPos).isConstant()) {
         * System.out.println(args + " (failed at pos " + argPos + ")");
         * System.out.println(children); return false; // -- yes } }
         */
        return true; // -- no
      }
